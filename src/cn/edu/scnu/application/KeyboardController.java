...
/**
 * 键盘控制器：用于双人模式中控制第二名玩家（键盘WASD控制左侧飞机）。
 * @author 黄彪骐、岳孝彬、丁俊哲
 */
public class KeyboardController extends KeyAdapter {
    ...
    public void updatePosition() {
        ...
        if (down) dy += speed;
        if (dx != 0 || dy != 0) {
            int hw = hero.getWidth() / 2;
            int hh = hero.getHeight() / 2;
            int nx = Math.max(hw, Math.min(Main.WINDOW_WIDTH - hw, hero.getLocationX() + dx));
            int ny = Math.max(hh, Math.min(Main.WINDOW_HEIGHT - hh, hero.getLocationY() + dy));
